    ...
    function resolveData(data) {
       ...
       //往页面写基础信息
        ...
        for (let i = 0; i < tracewater.length; i++) {
            ...
            $('#waterInfo #detail'+i+' #time').html(web3.toDecimal(tracewater[i][0]));
        ...
        for (let i = 0; i < transferInfo.length; i++) {
            ...
            $('#transferInfo #detail'+i+' #time').html(web3.toDecimal(tracetransfer[i][0]));
        ...
        var storeInsertHtml = '温度:<span id="temperature"></span><br>'+'湿度:<span id="wet"></span><br>'+'上链时间:<span id="time"></span><br>'+'上链人:<span id="operator"></span><br>'
        ...
        for (let i = 0; i < storeInfo.length; i++) {
            ...
            $('#storeInfo #detail'+i+' #temperature').html(web3.toDecimal(storeInfo[i][0]));
            $('#storeInfo #detail'+i+' #wet').html(web3.toDecimal(storeInfo[i][1]));
            $('#storeInfo #detail'+i+' #time').html(web3.toDecimal(tracestore[i][0]));
...
function insertHtml(insertPosition,insertContent,insertQuatity,type)
{
	...
	for(let i=0;i<insertQuatity;i++)
	{
		...
		else
		{
			...
			var position=i-1; 
			$('#'+insertAfter+' #detail'+position).after("<div id=detail"+haveInserted+"></div>");
		}
		haveInserted++;
